...
bool Launcher::Reset()
{
    ...
    if(get_systemTick()/1000 - start_time > 1000)
    {
        ...
        PushMotor[0].encoder_offset = PushMotor[0].get_encoder();
        machine_init_ = true;
    }
    else
    { 
...
{
    Motor_SendMsgs(&hcan1,PushMotor);
...
void Launcher::ShootCtrl(bool shoot_ready, bool friction_ready, float shoot_speed)
{
    if(shoot_ready)
    {
        PidPushSpd.target = PushPlanner.Plan(0,-1500,PushMotor[0].get_angle());
        PidPushSpd.current = PushMotor[0].get_speed();
        PushMotor[0].Out = PidPushSpd.Adjust();
    }
    else 
    {
        PidPushSpd.target = PushPlanner.Plan(-1500,0,PushMotor[0].get_angle());
        PidPushSpd.current = PushMotor[0].get_speed();
        PushMotor[0].Out = PidPushSpd.Adjust();
    }
    ...
    if(friction_ready)
    {
        ...
        FrictionMotor[1].Out = -shoot_speed*2.f/3.f;
    }
    else
    {
...
void Launcher::PushControl(float read_or_not)
{
        if(read_or_not)
        {
            PidPushSpd.target = PushPlanner.Plan(0,-900,PushMotor[0].get_angle());
            PidPushSpd.current = PushMotor[0].get_speed();
            PushMotor[0].Out = PidPushSpd.Adjust();
        }
        else 
        {
            PidPushSpd.target = PushPlanner.Plan(-900,0,PushMotor[0].get_angle());
            PidPushSpd.current = PushMotor[0].get_speed();
            PushMotor[0].Out = PidPushSpd.Adjust();
        }
}
